...
See CIRCUIT section for connections
...
The hardware for this project uses an Arduino:
 Connect an IR LED to pin 5 (IRLED).
 Connect a visible LED to the pin 7
 Connect a push-button between pin 5 (CENTER) and ground.
 Pin 4 (REGIONSWITCH) must be left floating for North America, or wire it to ground to have it output European codes.
...
void sendAllCodes() 
{
  ...
  // determine region from REGION: 1 = NA, 0 = EU (defined in main.h)
#if REGION == NA
    region = NA;
